...
class HttpTest(unittest.TestCase):
	...
	def testTunnel(self):
		self.assertRaises(ws.WebSocketProxyException, _tunnel, HeaderSockMock("data/header01.txt"), "example.com", 80,
		                  ("username", "password"))
		self.assertRaises(ws.WebSocketProxyException, _tunnel, HeaderSockMock("data/header02.txt"), "example.com", 80,
		                  ("username", "password"))
	...
	def testConnect(self):
		...
		if ws._http.HAVE_PYTHON_SOCKS:
			...
			self.assertRaises(ProxyTimeoutError, _start_proxied_socket, "wss://example.com", OptsList(),
			...
			self.assertRaises(ProxyConnectionError, connect, "wss://example.com", OptsList(),
			                  proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = 9999, proxy_type = "socks4",
			                             timeout = 1), None)
		...
		self.assertRaises(TypeError, _get_addrinfo_list, None, 80, True,
		                  proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = "9999", proxy_type = "http"))
		self.assertRaises(socket.timeout, connect, "wss://google.com", OptsList(),
		                  proxy_info(http_proxy_host = "8.8.8.8", http_proxy_port = 9999, proxy_type = "http",
		                             timeout = 1), None)
		self.assertEqual(
				connect("wss://google.com", OptsList(),
				        proxy_info(http_proxy_host = "8.8.8.8", http_proxy_port = 8080, proxy_type = "http"), True),
				(True, ("google.com", 443, "/")))
	...
	def testProxyConnect(self):
		...
		ws.connect("ws://127.0.0.1:" + LOCAL_WS_SERVER_PORT, http_proxy_host = "127.0.0.1", http_proxy_port = "8899",
		           proxy_type = "http")
		...
		self.assertEqual(_get_addrinfo_list("api.bitfinex.com", 443, True,
		                                    proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = "8899",
		                                               proxy_type = "http")),
		                 (socket.getaddrinfo("127.0.0.1", 8899, 0, socket.SOCK_STREAM, socket.SOL_TCP), True, None))
		self.assertEqual(connect("wss://api.bitfinex.com/ws/2", OptsList(),
		                         proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = 8899, proxy_type = "http"),
		                         None)[1], ("api.bitfinex.com", 443, '/ws/2'))
	...
	def testProxyInfo(self):
		self.assertEqual(
				...
				"http")
		self.assertRaises(ProxyError, proxy_info, http_proxy_host = "127.0.0.1", http_proxy_port = "8080",
		                  proxy_type = "badval")
		self.assertEqual(
		...
		self.assertEqual(
				...
				"8080")
		self.assertEqual(proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = "8080", proxy_type = "http").auth,
		                 None)
		self.assertEqual(proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = "8080", proxy_type = "http",
		                            http_proxy_auth = ("my_username123", "my_pass321")).auth[0], "my_username123")
		self.assertEqual(proxy_info(http_proxy_host = "127.0.0.1", http_proxy_port = "8080", proxy_type = "http",
		                            http_proxy_auth = ("my_username123", "my_pass321")).auth[1], "my_pass321")
